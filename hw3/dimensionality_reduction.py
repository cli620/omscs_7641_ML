...
class DimensionalityReduction(ABC):
    ...
    def visualize_components(self, x_reduced, y, dataset):
        ...
        mytitle = dataset
        periodidx = str.find(dataset, '.')
        slashidx = str.rfind(dataset, '/')
        startidx = str.find(dataset, './')
        if startidx >= 0:
            dataset = dataset[2:]
        if periodidx >= 0:
            mytitle = dataset[:periodidx]
        if slashidx >= 0:
            dataset = dataset[:slashidx]
            slashidx = str.rfind(dataset, '/')
            mytitle = dataset[slashidx+1:]

        utils.plot_components(component1, component2, df, self.name)
        utils.save_figure('{}_{}_components'.format(mytitle, self.name))
...
class IndependentComponents(DimensionalityReduction):
    ...
    def plot_model_complexity(self, x, dataset):
        ...
        if slashidx >= 0:
            ...
            slashidx = str.rfind(dataset, '/')
            mytitle = dataset[slashidx+1:]
        utils.save_figure_tight('{}_ica_model_complexity'.format(mytitle))
...
class KernelPrincipalComponents(DimensionalityReduction):
    ...
    def plot_model_complexity(self, x, dataset):
        ...
        if slashidx >= 0:
            ...
            slashidx = str.rfind(dataset, '/')
            mytitle = dataset[slashidx+1:]
        utils.save_figure_tight('{}_kpca_model_complexity'.format(mytitle))
...
class PrincipalComponents(DimensionalityReduction):
    ...
    def plot_model_complexity(self, x, dataset):
        ...
        if slashidx >= 0:
            ...
            slashidx = str.rfind(dataset, '/')
            mytitle = dataset[slashidx+1:]
        utils.save_figure_tight('{}_pca_model_complexity'.format(mytitle))
...
class RandomProjections(DimensionalityReduction):
    ...
    def __init__(self, n_components=2, random_runs=10, random_seed=42):
        ...
        self.model = GaussianRandomProjection(n_components=n_components, random_state=random_seed)
    ...
    def experiment(self, x_train, x_test, y_train, dataset, perform_model_complexity):
        ...
        for seed in range(self.random_seed, self.random_seed + self.random_runs):
            ...
            self.model = GaussianRandomProjection(n_components=self.n_components, random_state=seed)
    ...
    def plot_model_complexity(self, x, dataset):
        ...
        for seed in range(self.random_seed, self.random_seed + self.random_runs):
            ...
            for k in k_range:
                ...
                P_inv = np.linalg.pinv(rp.components_)  # dimensions check : k x m -> m x k
        ...
        if slashidx >= 0:
            ...
            slashidx = str.rfind(dataset, '/')
            mytitle = dataset[slashidx+1:]
        utils.save_figure('{}_rp_model_complexity'.format(mytitle))
    ...
    def reconstruct(self, x, x_reduced):
        ...
        # Compute Pseudo inverse, reconstruct data and compute MSE
        P_inv = np.linalg.pinv(self.model.components_)  # dimensions check : k x m -> m x k
        x_reconstructed = (P_inv @ x_reduced.T).T  # dimensions check: m x k x k x n = m x n -> n x m
