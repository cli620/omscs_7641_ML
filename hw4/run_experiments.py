    ...
    if frozen_flag:
        ...
        if vpq_flag[2]:
            for i in range(len(Adecay)):
                ...
                print('-' * 5, mapname, ' holeRew: ', holerewards[i], ' | eps decay: ', Edecay[i],
                      ' | learn rate decay: ', Adecay[i])
                frozen_lake_env = gym.make('CustomRewardedFrozenLake-v0', desc=my_map, map_name='25x25', rewarding=True,
                                           step_reward=-0.1, hole_reward=holerewards[i], is_slippery=False).unwrapped
                # frozen_lake_env.render()
                frozen_lake = MDP(env_name=mapname, environment=frozen_lake_env, convergence_threshold=0.0001,
                                  grid=True, max_iterations=1000)
                ...
                frozen_lake.Q_learning(num_episodes=num_ep[i], learning_rate_decay=Adecay[i], epsilon_decay=Edecay[i],
                                       visualize=True)
        else:
            mapname = 'FrozenLake'+str(n)+'x'+str(n)
            frozen_lake_env = gym.make('CustomRewardedFrozenLake-v0', desc=my_map, map_name='25x25', rewarding=True,
                                       step_reward=-0.1, hole_reward=-2, is_slippery=False).unwrapped
            # frozen_lake_env.render()
