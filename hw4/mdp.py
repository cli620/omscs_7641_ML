...
class MDP:
    ...
    def __init__(self, env_name, environment, convergence_threshold=0.00001, gamma=0.9, max_iterations=1e5, grid=False):
        self.env_name = env_name
        self.env = environment
        # gym.make('LunarLander-v2').unwrapped
            ...
            for state in range(self.env.nS):
                ...
                if visualize and policy is None and \
                   iteration == iterations_to_save[saving_index % len(iterations_to_save)]:
                    ...
                    # utils.plot_value_function(self.movingaverage(V, 25), iteration, show_label=True)
                    saving_index += 1
    ...
    def policy_iteration(self, iterations_to_save=[0], visualize=False):
        ...
        if visualize:
            ...
            # plotting the final policy iteration policy.
            plt.figure(figsize=(10, 10))
    ...
    def Q_learning(self, num_episodes=1000, learning_rate_decay=0.99, epsilon_decay=0.99, visualize=False):
        ...
        while True:
            ...
            while True:
                ...
                while not self.env.P[state][action]:
        ...
        print('goal was met: ', goal_ct, '/', total_ct, ' times')
        ...
        if visualize:
            ...
            utils.set_plot_title_labels(title='Q-Learning - Moving Average Score',
                                        x_label='Episodes', y_label='Moving Average Score')
            utils.save_figure('{}_QL_accumulated_reward_avg'.format(self.env_name))

            plt.figure(figsize=(10, 10))
            # utils.plot_value_function(average_scores, 0)
            utils.plot_value_function(movingaverage(scores,5), 0)
            utils.set_plot_title_labels(title='Q-Learning - Moving Average Score',
                                        x_label='Episodes', y_label='Moving Average Score')
            utils.save_figure('{}_QL_accumulated_reward_window5'.format(self.env_name))

            plt.figure(figsize=(10, 10))
            # utils.plot_value_function(average_scores, 0)
            utils.plot_value_function(movingaverage(scores,50), 0)
            utils.set_plot_title_labels(title='Q-Learning - Moving Average Score',
                                        x_label='Episodes', y_label='Moving Average Score')
            utils.save_figure('{}_QL_accumulated_reward_window50'.format(self.env_name))
            ...
            if self.grid:
                ...
                utils.plot_heatmap_value_function(self.optimal_V, self.env.nrow, self.env.ncol)
                utils.set_plot_title_labels(title='Q-Learning - Optimal State-Value-function', x_label='states', y_label='values')
            else:
                utils.plot_value_function(self.optimal_V, 0)
                utils.set_plot_title_labels(title='Q-Learning - Optimal State-Value-function', x_label='states', y_label='values')
            ...
            utils.set_plot_title_labels(title='Q-Learning - Optimal Policy', x_label='states', y_label='action taken')
